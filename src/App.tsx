          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="lg:col-span-5 flex flex-col gap-6">
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-xs flex flex-col gap-5">
                ...
                {/* Status Indicator with live Server Sync feedback */}
                <div className="bg-slate-50 dark:bg-slate-950 px-4 py-3 rounded-xl border border-slate-100 dark:border-slate-850 flex flex-col gap-2.5 text-xs">
                  <div className="flex items-center justify-between">
                    <span className="text-slate-400 font-semibold">Active Original Source:</span>
                    <span className="font-mono font-black text-indigo-650 dark:text-indigo-400 uppercase tracking-tight bg-indigo-50 dark:bg-indigo-950/40 px-2 py-0.5 rounded text-[10px]">
                      {lastSourceLoaded}
                    </span>
                  </div>
                  <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-800/50 pt-2.5 mt-1">
                    <span className="text-slate-400 font-semibold">Workspace Disk Sync:</span>
                    {syncStatus === "idle" && (
                      <span className="inline-flex items-center gap-1 font-mono text-[10px] text-emerald-600 dark:text-emerald-450 font-bold bg-emerald-50 dark:bg-emerald-950/30 px-1.5 py-0.5 rounded">
                        <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                        Synchronized
                      </span>
                    )}
                    {syncStatus === "syncing" && (
                      <span className="inline-flex items-center gap-1 font-mono text-[10px] text-amber-600 dark:text-amber-400 font-bold bg-amber-50 dark:bg-amber-950/30 px-1.5 py-0.5 rounded">
                        <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-ping" />
                        Saving files...
                      </span>
                    )}
                    {syncStatus === "success" && (
                      <span className="inline-flex items-center gap-1 font-mono text-[10px] text-emerald-700 dark:text-emerald-350 font-black bg-emerald-100 dark:bg-emerald-900/40 px-1.5 py-0.5 rounded">
                        <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-ping" />
                        Saved Successfully!
                      </span>
                    )}
                    {syncStatus === "error" && (
                      <span className="inline-flex items-center gap-1 font-mono text-[10px] text-rose-600 dark:text-rose-400 font-bold bg-rose-50 dark:bg-rose-950/30 px-1.5 py-0.5 rounded">
                  ...
                  <div className="border-t border-slate-100 dark:border-slate-800 pt-4 mt-2 flex flex-col gap-3">
                    <div>
                      <label className="text-[10px] uppercase font-black tracking-wider text-slate-400 block mb-1">
                        Save / Export XML Core Files
                      </label>
                      <p className="text-[11px] text-slate-400 leading-relaxed mb-3">
                        Retrieve the XML files representing your current custom tasks, categories, and titles.
                      </p>
                    </div>

                    <div className="flex flex-col gap-2">
                      <button
                        id="export-xml-file-btn"
                        type="button"
                        onClick={exportToXMLFile}
                        className="flex justify-center items-center gap-2.5 py-2.5 px-4 bg-indigo-650 hover:bg-indigo-700 text-white font-bold text-xs rounded-xl transition-all cursor-pointer shadow-xs"
                        title="Download checklist_presets.xml matching current tasks"
                      >
                        <Download className="w-3.5 h-3.5" />
                        <span>Download checklist_presets.xml</span>
                      </button>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <button
                          ...
                          className="flex justify-center items-center gap-1.5 py-2 px-3 bg-slate-100 hover:bg-slate-200 dark:bg-slate-850 dark:hover:bg-slate-800 text-slate-750 dark:text-slate-200 font-semibold text-[10px] rounded-lg border border-slate-200 dark:border-slate-800 transition-all cursor-pointer"
                          title="Download updated categories.xml config file"
                        >
                          <Layers className="w-3 h-3 text-slate-400" />
                          <span>categories.xml</span>
                        </button>
                        <button
                          type="button"
                          onClick={downloadConfigFile}
                          className="flex justify-center items-center gap-1.5 py-2 px-3 bg-slate-100 hover:bg-slate-200 dark:bg-slate-850 dark:hover:bg-slate-800 text-slate-750 dark:text-slate-200 font-semibold text-[10px] rounded-lg border border-slate-200 dark:border-slate-800 transition-all cursor-pointer"
                          title="Download updated app_config.xml config file"
                        >
                          <SlidersHorizontal className="w-3 h-3 text-slate-400" />
                          <span>app_config.xml</span>
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="mt-2 p-3.5 bg-indigo-50/50 dark:bg-slate-950/40 rounded-xl border border-indigo-100/50 dark:border-indigo-900/30 animate-fade-in">
                      ...
                      <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-relaxed mb-2.5">
                        Azure containers are temporary. Any tasks or categories added through this page reside in the live server's temporary ramdisk and will get erased on your next GitHub Actions push. To retain them forever:
                      </p>
                      <ol className="text-[10.5px] text-slate-650 dark:text-slate-3.5 space-y-1.5 pl-4 list-decimal leading-relaxed">
                        <li>
                          Modify tasks and categories here, then <strong className="text-slate-850 dark:text-white">download all three modified XML files</strong> above.
                        </li>
                        <li>
                          Copy/replace those downloaded files into the <code className="px-1 py-0.5 bg-slate-100 dark:bg-slate-800 rounded text-[9.5px]">src/</code> folder inside your local GitHub repository.
                        </li>
                        <li>
                          Commit and push the changes to <strong className="text-slate-850 dark:text-white">GitHub</strong>. Your Azure deployment pipeline will automatically update and boot permanently with your custom presets!
                        </li>
                      </ol>
                    </div>
                  </div>
